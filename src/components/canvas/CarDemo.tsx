...
const revoluteJointVehicleContext = createContext<RevoluteJointVehicleContext>(null!)

type RevoluteJointVehicleProps = JSX.IntrinsicElements['group'] & {
  children: React.ReactNode
}

const RevoluteJointVehicle = ({ children, ...groupProps }: RevoluteJointVehicleProps) => {
  ...
  const [steeredWheels, setSteeredWheels] = useState<RevoluteJointVehicleWheelInfo[]>([])
  const [torqueWheels, setTorqueWheels] = useState<RevoluteJointVehicleWheelInfo[]>([])
  ...
  useFrame(() => {
    // input
    const { forward, right, left, backward } = get()
    let forwardVel = 0
    let rightVel = 0
    ...
    if ((chassisRigidBody?.raw().isSleeping && rightVel !== 0) || forwardVel !== 0) {
    ...
    torqueWheels.forEach((wheel) => {
      ...
      wheelJoint.configureMotorVelocity(forwardVel, 10)
    })
  ...
  const registerChassis = (chassis: RigidBodyApi) => setChassisRigidBody(chassis)
...
const WheelColliderAndMesh: React.FC = () => (
  <>
    ...
    <CylinderCollider mass={1} friction={1.5} args={[0.15, 0.3]} rotation={[0, 0, Math.PI / 2]} />
  </>
)

const vectorArrayToVector3 = ([x, y, z]: Vector3Tuple): Rapier.Vector3 => new Rapier.Vector3(x, y, z)

const addVector3Arrays = (a: Vector3Array, b: Vector3Array): Vector3Array => [a[0] + b[0], a[1] + b[1], a[2] + b[2]]
  ...
  useEffect(() => {
    ...
    const wheelJoint = world.createImpulseJoint(
      rapier.JointData.revolute(
        vectorArrayToVector3(steered ? axleOffset : addVector3Arrays(anchor, axleOffset)),
        vectorArrayToVector3([0, 0, 0]),
        vectorArrayToVector3([1, 0, 0]),
      ),
      world.getRigidBody(steered ? axleRigidBody.current!.handle : vehicleContext.chassisRigidBody!.handle)!,
  ...
  return (
    <>
      ...
      <RigidBody
        ...
        ref={wheelRigidBody}
        colliders={false}
        position={!steered ? anchor : addVector3Arrays(anchor, axleOffset)}>
        {/* wheel colliders */}
        {children}
      </RigidBody>
      ...
      {/* wheel axle */}
      {steered ? (
        <RigidBody ref={axleRigidBody} colliders={false} position={addVector3Arrays(anchor, axleOffset)}>
...
const RevoluteJointVehicleChassis = ({ children, ...rigidBodyProps }: RevoluteJointVehicleChassisProps) => {
  const vehicleContext = useContext(revoluteJointVehicleContext)
  const [chassisRigidBody, setChassisRigidBody] = useState<RigidBodyApi | null>()
  ...
  return (
    <>
      <RigidBody {...rigidBodyProps} ref={setChassisRigidBody} type='dynamic' colliders={false}>
        {children}
      </RigidBody>
    </>
  )
...
function CarDemo() {
  return (
    <>
      <Physics gravity={[0, -9.81, 0]} updatePriority={RAPIER_UPDATE_PRIORITY}>
        <RevoluteJointVehicle position={[0, 3, 0]}>
          ...
          <RevoluteJointVehicleWheel anchor={[-0.75, -0.4, 1.2]} side='left' steered>
            <WheelColliderAndMesh />
          </RevoluteJointVehicleWheel>

          <RevoluteJointVehicleWheel anchor={[0.75, -0.4, 1.2]} side='right' steered>
            <WheelColliderAndMesh />
          </RevoluteJointVehicleWheel>

          <RevoluteJointVehicleWheel anchor={[-0.75, -0.4, -1.2]} side='left' torque>
            <WheelColliderAndMesh />
          </RevoluteJointVehicleWheel>

          <RevoluteJointVehicleWheel anchor={[0.75, -0.4, -1.2]} side='right' torque>
            <WheelColliderAndMesh />
          </RevoluteJointVehicleWheel>
