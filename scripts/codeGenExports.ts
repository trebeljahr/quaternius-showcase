...
async function codeGenForImports() {
  ...
  for (const packName of namesOfPacks) {
    ...
    for (const file of components) {
      ...
      contents = contents.replace(/<GLTFActions>/g, "");
...
interface GLTFAction extends THREE.AnimationClip {
  name: ActionName
      ...
      contents = contents.replace(
        new RegExp(`useGLTF\\('/${file}\\.glb`, "g"),
        `useGLTF('/glb/${packName}/${file}.glb`,
      );
      contents = contents.replace(/\) as GLTFResult/g, `) as unknown as GLTFResult`);
      contents = contents.replace(
        new RegExp(`useGLTF\\.preload\\('/${file}\\.glb`, "g"),
        `useGLTF.preload('/glb/${packName}/${file}.glb`,
      );

      contents = contents.replace(/\/glb\/glb\//g, `/glb/`);
      contents = contents.replace(
        new RegExp(`useGLTF\\('/glb/${packName}/glb/${packName}/${file}\\.glb`, "g"),
        `useGLTF('/glb/${packName}/${file}.glb`,
      );
      contents = contents.replace(/export function Model/g, `export default function Model`);
